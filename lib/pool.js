...
// Must exactly be 8 or 16 hex chars, already lowercased before test
var noncePattern8 = new RegExp("^[0-9a-f]{8}$");
var noncePattern16 = new RegExp("^[0-9a-f]{16}$");
...
var cryptoNightLite = multiHashing.cryptonight_light;
var k12 = multiHashing.k12;
...
Miner.prototype = {
    ...
    getTargetHex: function(version){
        ...
        var buff = version >= 8 ? padded.slice(0, 8) : padded.slice(0, 4);
    ...
    getJob: function(){
        ...
        var version = parseInt(blob.substr(0, 2), 16);
        var target = this.getTargetHex(version);
...
function processShare(miner, job, blockTemplate, nonce, resultHash){
    ...
    if (shareTrustEnabled && miner.trust.threshold <= 0 && miner.trust.penalty <= 0 && Math.random() > miner.trust.probability){
        ...
        var version = convertedBlob[0];
        if (version >= 8) {
            hash = k12(convertedBlob);
        }
        else {
            var cn_variant = version >= 7 ? version - 6 : 0;
            light = job.height >= 592000 || version >= 2;
            hash = light ? cryptoNightLite(convertedBlob, cn_variant) : cryptoNight(convertedBlob, cn_variant);
        }
        shareType = 'valid';
    }
...
function handleMinerMethod(method, params, ip, portData, sendReply, pushMessage){
    ...
    switch(method){
        ...
        case 'submit':
            ...
            params.nonce8 = params.nonce.substr(0, 8).toLowerCase();
            params.nonce16 = params.nonce.substr(0, 16).toLowerCase();
            if (!noncePattern8.test(params.nonce8) && !noncePattern16.test(params.nonce16)) {
            ...
            params.nonce = noncePattern16.test(params.nonce16) ? params.nonce16 : params.nonce8;
...
function startPoolServerTcp(callback){
    async.each(config.poolServer.ports, function(portData, cback){
        ...
        if (portData.tls === true) {
            tls.createServer({
                key: fs.readFileSync('cert.key'),
                cert: fs.readFileSync('cert.pem')
            }, socketConn).listen(portData.port, config.poolServer.pool_ip, function (error, result) {
                if (error) {
            ...
            net.createServer(socketConn).listen(portData.port, config.poolServer.pool_ip, function (error, result) {
                if (error) {
                    log('error', logSystem, 'Could not start server listening on port %d, error: $j', [portData.port, error]);
                    cback(true);
                    return;
